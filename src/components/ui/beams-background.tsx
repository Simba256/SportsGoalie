...
function buildGeometry(n: number, width: number, height: number, segs: number): THREE.BufferGeometry {
  ...
  for (let i = 0; i < n; i++) {
    ...
    for (let j = 0; j <= segs; j++) {
      ...
      uvs.set([ux, j / segs + uy, ux + 1, j / segs + uy], ui);
      if (j < segs) { indices.set([vi, vi+1, vi+2, vi+2, vi+1, vi+3], ii); ii += 6; }
      vi += 2; ui += 4;
    }
  }
...
const ShadowLight: FC<{ position: [number, number, number]; color: string }> = ({ position, color }) => {
  const ref = useRef<THREE.DirectionalLight>(null!);
  useEffect(() => {
    if (!ref.current) return;
    const cam = ref.current.shadow.camera as THREE.Camera & { top:number;bottom:number;left:number;right:number;far:number };
    cam.top = 24; cam.bottom = -24; cam.left = -24; cam.right = 24; cam.far = 64;
    ref.current.shadow.bias = -0.004;
  }, []);
  return <directionalLight ref={ref} color={color} intensity={1} position={position} />;
};
...
vec3 getNormal(vec3 pos){vec3 c=getCurrentPos(pos);vec3 tx=normalize(getCurrentPos(pos+vec3(0.01,0.,0.))-c);vec3 tz=normalize(getCurrentPos(pos+vec3(0.,-0.01,0.))-c);return normalize(cross(tz,tx));}`,
    fragmentHeader: '',
    vertex: {
    ...
    <div style={{ position: 'relative', overflow: 'hidden' }} className={className}>
      <CanvasWrapper>
        <group rotation={[0, 0, degToRad(rotation)]}>
          <Planes ref={meshRef} material={mat} count={beamNumber} width={beamWidth} height={beamHeight} />
          <ShadowLight color={lightColor} position={[0, 3, 10]} />
        </group>
