...
export default function CoachDashboardPage() {
  ...
  return (
    <div className="max-w-7xl mx-auto space-y-6">
      {/* ── Welcome Banner (matches student dashboard dark gradient) ── */}
      ...
      {user?.role === 'coach' && (
        <div className="rounded-2xl border border-gray-200 bg-white p-6 shadow-sm">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h2 className="text-lg font-bold text-gray-900">Your Coach Code</h2>
          </div>
          <p className="text-sm text-gray-500 mb-4">Share this code with students to connect with them</p>
          <div className="flex items-center gap-3">
            {coachCode ? (
              <>
                <code className="text-2xl font-mono font-bold tracking-wider bg-gray-50 px-5 py-2.5 rounded-xl border border-gray-200 text-gray-900">
                  {coachCode}
                </code>
                <button
                  onClick={handleCopyCode}
                  className="h-10 w-10 rounded-xl border border-gray-200 bg-gray-50 flex items-center justify-center hover:bg-gray-100 transition-colors"
                ...
                <button
                  ...
                  className="h-10 w-10 rounded-xl border border-gray-200 bg-gray-50 flex items-center justify-center hover:bg-gray-100 transition-colors disabled:opacity-50"
                  title="Regenerate code"
                >
                  <RefreshCw className={`h-4 w-4 text-gray-500 ${regenerating ? 'animate-spin' : ''}`} />
                </button>
              </>
            ) : (
              <button
      ...
      <div className="rounded-2xl border border-gray-200 bg-white p-6">
        ...
        <div className="flex flex-wrap gap-3">
          <Link href="/coach/students">
            <button className="bg-red-600 hover:bg-red-700 text-white px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 hover:shadow-lg hover:shadow-red-500/25 flex items-center gap-2">
              <Users className="h-4 w-4" />
              View All Students
            </button>
          </Link>
          <Link href="/coach/content">
            <button className="border-2 border-gray-200 bg-white hover:bg-gray-50 text-gray-700 px-5 py-2.5 rounded-xl font-semibold text-sm transition-all duration-300 flex items-center gap-2">
