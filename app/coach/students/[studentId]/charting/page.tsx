...
export default function CoachStudentChartingPage() {
  ...
  if (!student) {
    return (
      <div style={{ maxWidth: '560px', margin: '80px auto', padding: '0 16px', textAlign: 'center' }}>
        <p style={{ color: 'rgba(255,255,255,0.4)', fontSize: '14px', marginBottom: '16px' }}>Student not found</p>
  ...
  const initials = (student.displayName || student.email || 'G').split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase();
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: 'clamp(20px,3vw,32px) clamp(14px,3vw,24px) 56px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <div style={{ position: 'relative', background: cardBg, border, borderRadius: '16px', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid rgba(55,181,255,0.1)' }}>
            <h2 style={{ color: '#fff', fontWeight: 700, fontSize: '15px', marginBottom: '2px' }}>Game & Practice Charts</h2>
            <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '12px' }}>
              Review every session this goalie has charted. Expand a row to read the full reflection, Mind Vault entries, and ratings.
            </p>
          </div>
          <div style={{ padding: '16px' }}>
            <GoalieChartingHistory studentId={studentId} onOpenSession={(sessionId) => router.push(`/charting/sessions/${sessionId}`)} />
          </div>
        </div>
      </div>
    </>
