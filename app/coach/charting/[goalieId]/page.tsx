...
import { ArrowLeft, Trophy, Check, Plus, Loader2, Calendar, MapPin, ClipboardList } from 'lucide-react';
...
export default function CoachChartingGoaliePage() {
  const { user }  = useAuth();
  const router    = useRouter();
  const params    = useParams();
  const goalieId  = params.goalieId as string;
  ...
  useEffect(() => {
    if (!goalieId || !user) return;
    const load = async () => {
      setLoading(true);
      try {
        const result = await userService.getUser(goalieId);
        if (result.success && result.data) {
          if (user.role === 'coach' && result.data.assignedCoachId !== user.id) {
            toast.error('This goalie is not on your roster');
            router.push('/coach/charting');
            return;
          }
          setGoalie(result.data);
        } else {
    ...
    load();
  }, [goalieId, user, router]);

  // ── Load game sessions + existing coach charts ────────────────────────────
    ...
    const loadData = async () => {
      ...
      try {
        const [sessionsResult, chartsResult] = await Promise.all([
          chartingService.getSessionsByStudent(goalieId, {
            sessionType: 'game',
            orderBy: 'date',
            orderDirection: 'desc',
            limit: 50,
          }),
          coachChartingService.getChartsByStudent(goalieId, user.id),
        ]);
  ...
  if (!goalie) {
    return (
      <div style={{ maxWidth: '560px', margin: '80px auto', padding: '0 16px', textAlign: 'center' }}>
        ...
        <Link href="/coach/charting" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', background: 'rgba(55,181,255,0.08)', border: '1px solid rgba(55,181,255,0.2)', color: BLUE, padding: '9px 18px', borderRadius: '10px', textDecoration: 'none', fontWeight: 700, fontSize: '13px' }}>
          Back to Charting
        </Link>
      </div>
    );
  ...
  const initials = (goalie.displayName || goalie.email || 'G')
  ...
  const charted   = coachCharts.filter(c => c.completedSections.length === 3).length;
  const inProgress = coachCharts.filter(c => c.completedSections.length > 0 && c.completedSections.length < 3).length;
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: '900px', margin: '0 auto', padding: 'clamp(20px,3vw,32px) clamp(14px,3vw,24px) 56px', display: 'flex', flexDirection: 'column', gap: '24px' }}>
        ...
        <Link href="/coach/charting" className="sc-back" style={{ display: 'inline-flex', alignItems: 'center', gap: '6px', color: 'rgba(255,255,255,0.5)', textDecoration: 'none', fontSize: '13px', fontWeight: 600, borderRadius: '8px', padding: '6px 10px', width: 'fit-content', transition: 'all 0.2s' }}>
        ...
        <div style={{ position: 'relative', borderRadius: '20px', background: 'linear-gradient(135deg, #04213f 0%, #0b3460 50%, #0d1f40 100%)', border, boxShadow: '0 4px 32px rgba(0,0,0,0.5), inset 0 1px 0 rgba(55,181,255,0.12)', padding: '24px', overflow: 'hidden' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
            ...
            <div style={{ width: '60px', height: '60px', borderRadius: '50%', background: `linear-gradient(135deg, ${GOLD} 0%, #B8891E 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '18px', fontWeight: 800, color: '#0c0800', flexShrink: 0, border: '2px solid rgba(212,169,59,0.35)' }}>
              {goalie.profileImage
                ? <img src={goalie.profileImage} alt={goalie.displayName} style={{ width: '100%', height: '100%', borderRadius: '50%', objectFit: 'cover' }} />
                : initials}
            </div>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '4px' }}>
                ...
                <span style={{ color: GOLD, fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '1.5px' }}>My Charts</span>
              </div>
              <h1 style={{ color: '#fff', fontWeight: 800, fontSize: '22px' }}>{goalie.displayName}</h1>
              {goalie.email && <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '12px', marginTop: '2px' }}>{goalie.email}</p>}
            </div>

            {/* Stats */}
            <div style={{ display: 'flex', gap: '12px', flexShrink: 0 }}>
              <div style={{ textAlign: 'center', padding: '10px 16px', borderRadius: '12px', background: 'rgba(74,222,128,0.08)', border: '1px solid rgba(74,222,128,0.2)' }}>
                <p style={{ color: GREEN, fontSize: '20px', fontWeight: 800, margin: 0 }}>{charted}</p>
                <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '10px', fontWeight: 600, margin: '2px 0 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>Complete</p>
              </div>
              {inProgress > 0 && (
                <div style={{ textAlign: 'center', padding: '10px 16px', borderRadius: '12px', background: 'rgba(251,191,36,0.08)', border: '1px solid rgba(251,191,36,0.2)' }}>
                  <p style={{ color: '#fbbf24', fontSize: '20px', fontWeight: 800, margin: 0 }}>{inProgress}</p>
                  <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '10px', fontWeight: 600, margin: '2px 0 0', textTransform: 'uppercase', letterSpacing: '0.5px' }}>In Progress</p>
                </div>
              )}
            </div>
          </div>
        ...
        <div style={{ position: 'relative', background: cardBg, border, borderRadius: '16px', overflow: 'hidden' }}>
          ...
          <div style={{ padding: '18px 20px 14px', borderBottom: '1px solid rgba(55,181,255,0.1)' }}>
            <h2 style={{ color: '#fff', fontWeight: 700, fontSize: '15px', marginBottom: '2px' }}>Game Sessions</h2>
            <p style={{ color: 'rgba(255,255,255,0.35)', fontSize: '12px' }}>
              Chart any game below. Your evaluation cross-references with the goalie&rsquo;s self-assessment — alignment confirms signals, gaps reveal where coaching begins.
            </p>
          </div>
                ...
                {gameSessions.map(session => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {/* Action button — navigates to the existing coach chart form */}
                      <button
                        type="button"
                        className="cc-btn"
                        onClick={() => router.push(`/coach/students/${goalieId}/charting/sessions/${session.id}`)}
                        style={{
