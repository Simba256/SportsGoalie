...
export function GoalieInviteList({ invitations, loading, onResend, onRevoke, onDelete }: Props) {
            ...
            <div style={{ background: 'rgba(255,255,255,0.04)', border: `1px solid ${isConfirmingDelete ? 'rgba(248,113,113,0.3)' : 'rgba(96,205,255,0.12)'}`, borderRadius: isConfirmingDelete ? '10px 10px 0 0' : '10px', padding: '14px 16px', display: 'grid', gridTemplateColumns: '1fr auto', gap: '12px', alignItems: 'center', transition: 'border-color 0.2s' }}>
              ...
              <div style={{ minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap', marginBottom: '6px' }}>
                  ...
                  {inv.metadata.tier && (
                    <span style={{ fontSize: '10px', fontWeight: 700, letterSpacing: '1px', textTransform: 'uppercase', color: inv.metadata.tier === 'custom' ? '#fbbf24' : 'rgba(255,255,255,0.35)', background: inv.metadata.tier === 'custom' ? 'rgba(251,191,36,0.12)' : 'rgba(255,255,255,0.06)', border: `1px solid ${inv.metadata.tier === 'custom' ? '#fbbf2433' : 'rgba(255,255,255,0.08)'}`, borderRadius: '20px', padding: '2px 8px' }}>
                      {inv.metadata.tier}
                    </span>
                  )}
                </div>
                <div style={{ display: 'flex', gap: '16px', flexWrap: 'wrap', fontSize: '12px', color: 'rgba(255,255,255,0.45)' }}>
                  ...
                  {inv.metadata.assignedCoachName && (
                    <span><ChevronRight size={10} style={{ display: 'inline', opacity: 0.5 }} />{inv.metadata.assignedCoachName}</span>
                  )}
